...
const PROVIDER_SIGNATURE_ERROR_MESSAGE =
  "The AI provider returned an internal validation error. Please try again in a moment.";
const PROVIDER_SIGNATURE_ERROR_RE =
  /thought signature is not valid|unable to submit request because thought signature is not valid/i;
...
function isProviderSignatureError(raw: string): boolean {
  return PROVIDER_SIGNATURE_ERROR_RE.test(raw);
}
...
export function formatRawAssistantErrorForUi(raw?: string): string {
  ...
  if (isProviderSignatureError(trimmed)) {
    return PROVIDER_SIGNATURE_ERROR_MESSAGE;
  }
  ...
  if (info?.message) {
    if (isProviderSignatureError(info.message)) {
      return PROVIDER_SIGNATURE_ERROR_MESSAGE;
    }
...
export function formatAssistantErrorText(
  ...
  if (isProviderSignatureError(raw)) {
    return PROVIDER_SIGNATURE_ERROR_MESSAGE;
  }
