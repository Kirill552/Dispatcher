#!/usr/bin/env bash
# E2E диалоговый тест через OpenClaw Chat Completions.
# Цель: быстро проверить качество ответов в сценариях "клиент" и "перевозчик",
# зафиксировать транскрипт и поймать явные анти-паттерны.

set -euo pipefail

if [[ $# -lt 2 ]]; then
  echo "Использование: $0 <gateway_url> <gateway_token> [client_session_key] [carrier_session_key]"
  echo "Пример: $0 http://127.0.0.1:18790 <TOKEN> agent:main:telegram:direct:90000000101 agent:main:telegram:direct:90000000102"
  exit 2
fi

GATEWAY_URL="$1"
GATEWAY_TOKEN="$2"
CLIENT_SESSION_KEY="${3:-agent:main:telegram:direct:90000000101}"
CARRIER_SESSION_KEY="${4:-agent:main:telegram:direct:90000000102}"

GATEWAY_TOKEN="${GATEWAY_TOKEN//$'\r'/}"

OUT_DIR="reference/tests/out"
mkdir -p "${OUT_DIR}"
OUT_FILE="${OUT_DIR}/dialog_flow_$(date +%Y%m%d_%H%M%S).log"
...
chat_turn() {
  ...
  if [[ "${code}" == "404" || "${code}" == "405" ]]; then
    echo "__CHAT_ENDPOINT_DISABLED__"
    return 10
  fi
...
run_dialog() {
  ...
  local -a banned_patterns=(
    ...
    "ATI API"
    "Exec:"
    "Role \"guest\" does not have access to tool \"exec\""
  ...
  for user_text in "$@"; do
    ...
    answer="$(chat_turn "${session_key}" "${user_text}")" || {
      if [[ "$?" -eq 10 ]]; then
        echo "SKIP: endpoint /v1/chat/completions выключен (404/405)."
        echo "Подсказка: включите gateway.http.endpoints.chatCompletions.enabled=true"
        exit 0
      fi
    ...
    for pattern in "${banned_patterns[@]}"; do
      if grep -Fqi "${pattern}" <<<"${answer}"; then
        echo "WARN: ${role} -> найден анти-паттерн '${pattern}'"
...
run_dialog "клиент" "${CLIENT_SESSION_KEY}" \
  "/start" \
  "Нужно перевезти диван Москва Казань" \
  "Вес 300 кг, объем 3 куба, загрузка завтра" \
  "Адрес: Каширское шоссе 120" \
  "Оплата без торга" || client_bad=$?

run_dialog "перевозчик" "${CARRIER_SESSION_KEY}" \
  "Здравствуйте, я перевозчик. Есть машина Москва-Казань." \
  "Готов взять груз, ставка 25000, тент." \
  "Мой ATI id 777.0, куда отправить детали?" || carrier_bad=$?

echo "Итог: анти-паттерны клиент=${client_bad}, перевозчик=${carrier_bad}"
echo "Проверьте лог: ${OUT_FILE}"

if [[ "${client_bad}" -gt 0 || "${carrier_bad}" -gt 0 ]]; then
  echo "FAIL: найдены анти-паттерны в ответах."
  exit 1
fi

echo "OK: критичных анти-паттернов не найдено."
